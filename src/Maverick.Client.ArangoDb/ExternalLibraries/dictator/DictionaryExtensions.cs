...
public static class DictionaryExtensions
{
    ...
    private static bool HasField(Dictionary<string, object> dictionary, string fieldPath)
    {
        ...
        string[] fieldNames = { fieldPath };
        Dictionary<string, object> parentDictionary = dictionary;
        ...
        for (var i = 0; i < fieldNames.Length; i++)
        {
            ...
            if (fieldName.Contains("[") && fieldName.Contains("]"))
            {
                ...
                int lastIndex = fieldName.IndexOf(']');
            ...
            object tempParentObject = GetFieldObject(fieldName, arrayContent, parentDictionary);
    ...
    private static object GetFieldObject(
        string fieldName,
        string arrayContent,
        Dictionary<string, object> fieldDocument)
    {
        if (!string.IsNullOrEmpty(arrayContent))
        {
            if (int.TryParse(arrayContent, out int indexNumber))
            {
    ...
    {
        object stronglyTypedObject = Activator.CreateInstance(objectType);
            ...
            foreach (PropertyInfo propertyInfo in objectType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                if (propertyInfo.PropertyType == typeof(Dictionary<string, object>))
                {
                    propertyInfo.SetValue(
                        stronglyTypedObject,
                        ConvertToObject((Dictionary<string, object>)fieldValue, propertyInfo.PropertyType),
                        null);
                }
                ...
                else if (propertyInfo.PropertyType.IsEnum)
                {
                             ...
                             || fieldValue is ulong)
                    {
                        propertyInfo.SetValue(
                            stronglyTypedObject,
                            System.Enum.ToObject(propertyInfo.PropertyType, fieldValue),
                            null);
                    }
                    ...
                    else if (fieldValue is string)
                    {
                        propertyInfo.SetValue(
                            stronglyTypedObject,
                            System.Enum.Parse(propertyInfo.PropertyType, (string)fieldValue, true),
                            null);
                    }
                }
                ...
                else
                {
                    ...
                    else
                    {
                        propertyInfo.SetValue(
                            stronglyTypedObject,
                            Convert.ChangeType(fieldValue, propertyInfo.PropertyType),
                            null);
                    }
                }
            }
                ...
                else if (collectionType.IsGenericType && collection is IEnumerable)
                {
                    ...
                    else
                    {
                        ...
                        else if (elementType == typeof(Dictionary<string, object>))
                        {
                            ((IList)collectionInstance).Add(
                                ConvertToObject(
                                    (Dictionary<string, object>)collection[i],
    ...
    /// </summary>
    public static Dictionary<string, object> Each(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        Action<int, Dictionary<string, object>> action)
    {
        object fieldValue = GetFieldValue(dictionary, fieldPath);
    ...
    /// </summary>
    public static Dictionary<string, object> Each<T>(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        Action<int, T> action)
    {
    ...
    public static Dictionary<string, object> Clone(this Dictionary<string, object> dictionary)
    {
        ...
        foreach (KeyValuePair<string, object> field in dictionary)
        {
            ...
            if (fieldType.IsValueType || fieldType.IsEnum || fieldType.Equals(typeof(string)))
            {
                clone.Add(field.Key, field.Value);
            }
            else if (fieldType == typeof(Dictionary<string, object>))
            {
                clone.Add(field.Key, Clone((Dictionary<string, object>)field.Value));
            }
            else
            {
                clone.Add(field.Key, Activator.CreateInstance(fieldType, field.Value));
            }
        }
    ...
    /// </summary>
    public static Dictionary<string, object> CloneExcept(
        this Dictionary<string, object> dictionary,
        params string[] fieldPaths)
    {
        Dictionary<string, object> clone = Clone(dictionary);
    ...
    /// </summary>
    public static Dictionary<string, object> CloneOnly(
        this Dictionary<string, object> dictionary,
        params string[] fieldPaths)
    {
        var clone = new Dictionary<string, object>();
        ...
        foreach (string fieldPath in fieldPaths)
        {
            try
            {
                ...
                {
                    clone.Object(fieldPath, Activator.CreateInstance(fieldType, fieldValue));
                }
            }
            catch (Exception)
            {
    ...
    /// </summary>
    public static Dictionary<string, object> Drop(
        this Dictionary<string, object> dictionary,
        params string[] fieldPaths)
    {
        foreach (string fieldPath in fieldPaths)
        {
            ...
            for (var i = 0; i < fieldNames.Length; i++)
            {
                ...
                // descendant field is dictionary - set is as current parent dictionary
                if (tempParentObject is Dictionary<string, object> objects)
                {
                    parentDictionary = objects;
                }
    ...
    public static Guid Guid(this Dictionary<string, object> dictionary, string fieldPath)
    {
        ...
        {
            throw new InvalidFieldTypeException($"Field path '{fieldPath}' value does not contain Guid type.");
        }
    ...
    public static Dictionary<string, object> Document(this Dictionary<string, object> dictionary, string fieldPath)
    {
        ...
        if (!(fieldValue is Dictionary<string, object>))
        {
            throw new InvalidFieldTypeException(
                string.Format(
                    "Field path '{0}' value does not contain Dictionary<string, object> type.",
                    fieldPath));
        }
    ...
    /// <summary>
    ///     Retrieves enum type value from specified field path. Value is converted to enum if it is byte, sbyte, short,
    ///     ushort, int, uint, long, ulong or string type.
    /// </summary>
    /// <exception cref="NonExistingFieldException">Field does not exist in specified path.</exception>
    /// <exception cref="InvalidFieldException">Field path contains field which is not traversable.</exception>
    ...
    public static T Enum<T>(this Dictionary<string, object> dictionary, string fieldPath)
    {
        ...
        else
        {
            throw new InvalidFieldTypeException(
                string.Format(
                    "Field path '{0}' value does not contain Enum type.",
                    fieldPath));
        }
    ...
    public static List<T> List<T>(this Dictionary<string, object> dictionary, string fieldPath)
    {
        ...
        if (!(fieldValue.GetType().IsGenericType && fieldValue is IEnumerable))
        {
            throw new InvalidFieldTypeException(
                string.Format(
                    "Field path '{0}' value does not contain list type.",
                    fieldPath));
        }

        var collection = (IList)fieldValue;
        Type collectionElementType = collection.GetType().GetGenericArguments()[0];
    ...
    public static T[] Array<T>(this Dictionary<string, object> dictionary, string fieldPath)
    {
        ...
        if (!(fieldValue is IEnumerable))
        {
            throw new InvalidFieldTypeException(
                string.Format(
                    "Field path '{0}' value does not contain array type.",
                    fieldPath));
        }

        var collection = (IList)fieldValue;
        Type collectionElementType = collection.GetType().GetElementType();
        ...
        object returnCollection = Activator.CreateInstance(typeof(T[]), collection.Count);
    ...
    public static int Size(this Dictionary<string, object> dictionary, string fieldPath)
    {
        ...
        if (!(fieldValue is IList))
        {
            throw new InvalidFieldTypeException(
                string.Format(
                    "Field path '{0}' value does not contain type which can retrieve items count.",
                    fieldPath));
        }
    ...
    /// </summary>
    public static Dictionary<string, object> Bool(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        bool fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Byte(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        byte fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Short(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        short fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Int(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        int fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Long(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        long fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Float(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        float fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Double(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        double fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Decimal(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        decimal fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> DateTime(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        DateTime fieldValue)
    {
        ...
        string dateTimeStringFormat)
    {
        SetFieldValue(
            dictionary,
            fieldPath,
            fieldValue.ToUniversalTime().ToString(dateTimeStringFormat, DateTimeFormatInfo.InvariantInfo));
        ...
        switch (dateTimeFormat)
        {
            case DateTimeFormat.String:
                SetFieldValue(
                    dictionary,
                    fieldPath,
                    fieldValue.ToUniversalTime()
                        .ToString(Dictator.Settings.DateTimeStringFormat, DateTimeFormatInfo.InvariantInfo));
    ...
    /// </summary>
    public static Dictionary<string, object> Guid(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        Guid fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Object<T>(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        T fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Enum<T>(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        T fieldValue)
    {
        ...
        switch (enumFormat)
        {
            case EnumFormat.Integer:
                SetFieldValue(
                    dictionary,
                    fieldPath,
                    (int)Convert.ChangeType(fieldValue, Type.GetTypeCode(fieldValue.GetType())));
    ...
    /// </summary>
    public static Dictionary<string, object> List<T>(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        List<T> fieldValue)
    {
    ...
    /// </summary>
    public static Dictionary<string, object> Array<T>(
        this Dictionary<string, object> dictionary,
        string fieldPath,
        T[] fieldValue)
    {
        ...
        try
        {
            ...
            switch (dateTimeFormat)
            {
                case DateTimeFormat.String:
                    if (fieldValue is string)
                    {
                        DateTime dateTime = System.DateTime.Parse(
                            (string)fieldValue,
                            DateTimeFormatInfo.InvariantInfo,
                            DateTimeStyles.AdjustToUniversal);
                ...
                case DateTimeFormat.UnixTimeStamp:
                    if (fieldValue is long)
                    {
                        DateTime dateTime = Dictator.Settings.UnixEpoch.AddSeconds((long)fieldValue);
    ...
    /// <summary>
    ///     Checks if specified field has integer value. Applicable integer types are byte, sbyte, short, ushort, int, uint,
    ///     long and ulong.
    /// </summary>
        ...
        for (var i = 0; i < fieldNames.Length; i++)
        {
            ...
            if (!parentDictionary.ContainsKey(fieldName))
            {
                throw new NonExistingFieldException(
                    string.Format(
                        "Field path '{0}' does not contain field '{1}'.",
                        fieldPath,
                        fieldName));
            }
            ...
            else
            {
                throw new InvalidFieldException(
                    string.Format(
                        "Field path '{0}' contains field '{1}' which is not dictionary.",
                        fieldPath,
                        fieldName));
            }
        }
