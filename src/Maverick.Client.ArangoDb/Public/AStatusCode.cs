...
/// <summary>
///     Arango specified error codes.
/// </summary>
public enum AStatusCode
{
    ...
    /// </summary>
    [ErrorClassification(AErrorSeverity.Fatal)]
    ErrorForbidden = 11,
    ...
    /// <summary>
    ///     Will be raised when theres a timeout waiting for a lock.
    /// </summary>
    ErrorLockTimeout = 18,
    ...
    /// </summary>
    [ErrorClassification(AErrorSeverity.Hint)]
    ...
    #region External ArangoDB storage
    ...
    /// <summary>
    ///     Will be raised when the servers database directory is not writable for the current user.
    /// </summary>
    [ErrorClassification(AErrorSeverity.Fatal)]
    ...
    /// </summary>
    ErrorClusterFollowerTransactionCommitPerformed = 1447,
    ...
    /// <summary>
    ///     Will be raised when a Coordinator in a cluster cannot create an entry for a new collection in the Plan hierarchy in
    ///     the Agency.
    /// </summary>
    ErrorClusterCouldNotCreateCollectionInPlan = 1454,

    /// <summary>
    ///     Will be raised when a Coordinator in a cluster notices that some DB-Servers report problems when creating shards
    ...
    /// <summary>
    ///     Will be raised when a Coordinator in a cluster cannot remove an entry for a collection in the Plan hierarchy in the
    ///     Agency.
    /// </summary>
    ErrorClusterCouldNotRemoveCollectionInPlan = 1458,

    /// <summary>
    ///     Will be raised when a Coordinator in a cluster cannot create an entry for a new database in the Plan hierarchy in
    ///     the Agency.
    /// </summary>
    ErrorClusterCouldNotCreateDatabaseInPlan = 1460,

    /// <summary>
    ///     Will be raised when a Coordinator in a cluster notices that some DB-Servers report problems when creating databases
    ///     for a new cluster wide database.
    /// </summary>
    ErrorClusterCouldNotCreateDatabase = 1461,

    /// <summary>
    ///     Will be raised when a Coordinator in a cluster cannot remove an entry for a database in the Plan hierarchy in the
    ///     Agency.
    /// </summary>
    ErrorClusterCouldNotRemoveDatabaseInPlan = 1462,

    /// <summary>
    ///     Will be raised when a Coordinator in a cluster cannot remove an entry for a database in the Current hierarchy in
    ///     the Agency.
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised when a Coordinator in a cluster finds that the _key attribute was specified in a sharded collection
    ///     the uses not only _key as sharding attribute.
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised if a required DB-Server cant be reached.
    /// </summary>
    ErrorClusterBackendUnavailable = 1478,

    /// <summary>
    ///     Will be raised if a collection needed during query execution is out of sync. This currently can only happen when
    ///     using SatelliteCollections
    /// </summary>
    ErrorClusterAqlCollectionOutOfSync = 1481,

    /// <summary>
    ///     Will be raised when a Coordinator in a cluster cannot create an entry for a new index in the Plan hierarchy in the
    ///     Agency.
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised if one tries to create a collection with a distributeShardsLike attribute which points to another
    ///     collection that also has one.
    /// </summary>
    ErrorClusterChainOfDistributeShardsLike = 1484,

    /// <summary>
    ///     Will be raised if one tries to drop a collection to which another collection points with its distributeShardsLike
    ///     attribute.
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised if one tries to create a collection with a replicationFactor greater than the available number of
    ///     DB-Servers.
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised if a replication operation is refused by a shard follower because it is coming from the wrong
    ///     leader.
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised when a Coordinator in a cluster cannot create an entry for a new View in the Plan hierarchy in the
    ///     Agency.
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised when a bind parameter was declared in the query but the query is being executed with no value for
    ///     that parameter.
    /// </summary>
    ...
    /// </summary>
    ErrorQueryCollectionUsedInExpression = 1568,
    ...
    /// <summary>
    ///     Will be raised when an AQL data-modification query contains options that cannot be figured out at query compile
    ///     time.
    /// </summary>
    ...
    #region AQL user function errorsPermalink
    ...
    /// </summary>
    ErrorQueryWindowAfterModification = 1595,
    ...
    /// <summary>
    ///     Will be raised when a wrong usage of transactions is detected. this is an internal error and indicates a bug in
    ///     ArangoDB.
    /// </summary>
    ...
    /// <summary>
    ///     LDAP didnt found any objects with the specified search query
    /// </summary>
    ...
    /// <summary>
    ///     during the graph creation process no collection could be selected as the needed initial collection. Happens if a
    ///     distributeShardsLike or replicationFactor mismatch was found.
    /// </summary>
    ErrorGraphNoInitialCollection = 1946,

    /// <summary>
    ///     the _from or _to collection specified for the edge refers to a vertex collection which is not used in any edge
    ///     definition of the graph.
    /// </summary>
    ErrorGraphReferencedVertexCollectionNotUsed = 1947,

    /// <summary>
    ///     a negative edge weight was found during a weighted graph traversal or shortest path query
    /// </summary>
    ErrorGraphNegativeEdgeWeight = 1948,
    ...
    /// <summary>
    ///     Will be raised when not enough healthy.
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised if a shard in collection and its prototype in the corresponding distributeShardsLike collection have
    ///     mismatching leaders (when they should already have been fixed)
    /// </summary>
    ...
    ///     dont have the same followers (when they should already have been adjusted)
    /// </summary>
    ...
    /// <summary>
    ///     Will be raised if in a collection and its distributeShardsLike prototype collection some shard and its prototype
    ///     have an unequal number of DB-Servers
    /// </summary>
    ...
    /// </summary>
    ErrorAgencyMalformedGossipMessage = 20001,

    /// <summary>
    ///     Malformed inquire request.
    /// </summary>
    ErrorAgencyMalformedInquireRequest = 20002,
    ...
    /// <summary>
    ///     The inform message in the Agency must contain an object timeout Mult.
    /// </summary>
    ...
    /// </summary>
    ErrorAgencyMalformedTransaction = 20030,
