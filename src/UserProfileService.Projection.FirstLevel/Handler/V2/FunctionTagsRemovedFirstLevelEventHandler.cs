...
/// <summary>
///     This handler is used to process <see cref="FunctionTagsRemovedEvent" />.
/// </summary>
internal class FunctionTagsRemovedFirstLevelEventHandler : FirstLevelEventHandlerBase<FunctionTagsRemovedEvent>
{
    ...
    /// <summary>
    ///     Creates an instance of the object <see cref="FunctionTagsRemovedFirstLevelEventHandler" />.
    /// </summary>
    ...
    public FunctionTagsRemovedFirstLevelEventHandler(
        ILogger<FunctionTagsRemovedFirstLevelEventHandler> logger,
    ...
    protected override async Task HandleInternalAsync(
        FunctionTagsRemovedEvent eventObject,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (string tagToRemoveId in tagsToRemoveIds)
        {
            ...
            try
            {
                await Repository.RemoveTagFromFunctionAsync(tag.Id, function.Id, transaction, cancellationToken);
            }
            catch (InstanceNotFoundException e)
            {
                Logger.LogErrorMessage(e, "Unable to remove tag from function.", LogHelpers.Arguments());
            }
            catch (Exception e)
            {
                Logger.LogError(e, "Unable to remove tag from function. Saga service will be aborted.");
                await _SagaService.AbortBatchAsync(batchSagaId, cancellationToken);
