...
using UserProfileService.Saga.Worker.Extensions;
using UserProfileService.Saga.Worker.States.Exceptions;
...
public class CommandProcessStateMachine :
    MassTransitStateMachine<CommandProcessState>
{
    ...
    /// <returns></returns>
    private static SubmitCommandSuccess SubmitCommandSuccess(
        BehaviorContext<CommandProcessState, CommandProjectionSuccess> context)
    {
    ...
    private static SubmitCommandFailure
        GenerateFailureMessage<TCommand>(
            BehaviorExceptionContext<CommandProcessState, TCommand, Exception> context)
    ...
    private static bool IsValidValidationResult(
        SagaConsumeContext<CommandProcessState, ValidationCompositeResponse> context)
    {
    ...
    private async Task ExecuteCommandEventAsync(
        SagaConsumeContext<CommandProcessState, ValidationCompositeResponse> context)
    {
        _logger.EnterMethod();
