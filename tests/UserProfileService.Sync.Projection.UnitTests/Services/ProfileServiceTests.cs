...
namespace UserProfileService.Sync.Projection.UnitTests.Services
{
    public class ProfileServiceTests
    {
        ...
        [Theory]
        [AutoData]
        public async Task GetRoleAsync_should_work(RoleSync role)
        {
            //Arrange
            ILogger<ProfileService> logger = new LoggerFactory().CreateLogger<ProfileService>();
            var profileStore = new Mock<IEntityStore>();
            ...
            profileStore.Setup(p => p.GetRoleAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
                        .ReturnsAsync(role);

            var profileService = new ProfileService(logger, profileStore.Object, projectionStateRepository.Object);

            //Act
            RoleSync repoUser = await profileService.GetRoleAsync(role.Id);

            //Assert
            profileStore.Verify(
                p => p.GetRoleAsync(It.Is<string>(i => i == role.Id), It.IsAny<CancellationToken>()),
                Times.Once);

            repoUser.Should().BeEquivalentTo(role);
        }

        [Theory]
        [AutoData]

        public async Task GeFunctionAsync_should_work(FunctionSync function)
        {
            //Arrange
            ILogger<ProfileService> logger = new LoggerFactory().CreateLogger<ProfileService>();
            var profileStore = new Mock<IEntityStore>();
            ...
            profileStore.Setup(p => p.GetFunctionAsync(It.IsAny<string>(), ct))
                        .ReturnsAsync(function);

            var profileService = new ProfileService(logger, profileStore.Object, projectionStateRepository.Object);

            //Act
            FunctionSync repoUser = await profileService.GetFunctionAsync(function.Id, ct);

            //Assert
            profileStore.Verify(
                p => p.GetFunctionAsync(It.Is<string>(i => i == function.Id), ct),
                Times.Once);

            repoUser.Should().BeEquivalentTo(function);
        }
        ...
        [Theory]
        [AutoData]
        public async Task CreateProfileAsync(GroupSync group)
        {
            //Arrange
            ILogger<ProfileService> logger = new LoggerFactory().CreateLogger<ProfileService>();
            var profileStore = new Mock<IEntityStore>();
            ...
            //Act
            GroupSync repoGroup = await profileService.CreateProfileAsync(group);
